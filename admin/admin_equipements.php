...
if (isset($_GET['delete'])) {
    $equipId = intval($_GET['delete']);
    mysqli_query($conn, "DELETE FROM equipements WHERE equipement_id = $equipId");
    $_SESSION['success'] = "Équipement supprimé avec succès";
    header('Location: admin_equipements.php');
    exit();
}

$equipements = mysqli_query($conn, "SELECT * FROM equipements ORDER BY statut, nom");
...
<body>
    ...
    <main class="container mb-5">
        <div class="d-flex justify-content-between align-items-center mb-4">
            <h2><i class="bi bi-pc-display"></i> Gestion des Équipements</h2>
            <a href="admin_add_equipement.php" class="btn btn-primary">
                ...
                <div class="table-responsive">
                    <table class="table table-hover">
                        ...
                        <tbody>
                            ...
                            <tr>
                                <td><?= $equip['equipement_id'] ?></td>
                                <td><?= htmlspecialchars($equip['nom']) ?></td>
                                <td><?= htmlspecialchars($equip['description']) ?></td>
                                <td>
                                    <span class="badge bg-<?= 
                                        $equip['statut'] == 'actif' ? 'success' : 
                                        ($equip['statut'] == 'en_panne' ? 'danger' : 'warning') 
                                    ?>">
                                        <?= ucfirst($equip['statut']) ?>
                                    </span>
                                </td>
                                <td><?= $equip['date_ajout'] ?></td>
                                <td>
                                    <a href="admin_edit_equipement.php?id=<?= $equip['equipement_id'] ?>" class="btn btn-sm btn-primary">
                                        <i class="bi bi-pencil"></i>
                                    </a>
                                    <a href="admin_equipements.php?delete=<?= $equip['equipement_id'] ?>" 
